	...
	: AuthenticationStateProvider, IAuthService 
{
	...
	public async Task<LoginResult> Login(LoginModel loginModel)
	{
		...
		{
			alertService.ShowAlert("Error.", "Login failed. Please check your credentials and try again.");
		}
	...
	private ClaimsIdentity ParseClaimIdentityFromJwt(string jwt) {
		...
		if (expirationDate > nowTimeProvider.Now)
		{
	...
	public async Task<AuthenticationHeaderValue> GetAuthHeader() {
		return new AuthenticationHeaderValue("bearer", await localStorageService.GetItemAsync<string>("authToken"));
	}
